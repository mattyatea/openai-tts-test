...
- ストリーミング再生。Irodori のチャンクは届いた順にそのまま鳴らす
...
| `tts.stream` | SSE を受けてチャンクが届くたびに流す |
...
`scripts/stream-smoke.mjs` は、チャンクの到着時刻を測って逐次配信を確認します。
...
Codex 本体の実装（`codex-rs/core/src/realtime_prompt.rs`）では、realtime セッションの
システムプロンプトは `thread/realtime/start` の `prompt` 引数がそのまま使われます。
未指定のときだけ `You are Codex, an OpenAI general-purpose agentic assistant` という
既定文が入ります。

このアプリは `systemPrompt` を `prompt` として渡し、既定の Codex 人格を置き換えます。
`instructions` は別枠の developer 指示（`realtimeStartInstructions`）として会話コンテキストに足します。

セッション開始後、「実際に realtime へ渡したシステムプロンプト」を開くと送信内容を確認できます。
